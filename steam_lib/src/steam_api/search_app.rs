...
impl SteamGame {
    ...
    */
    pub fn from_appid(app_id: u64) -> Result<SteamGame, Error> {
        let response = blocking::get(format!("https://store.steampowered.com/api/appdetails?appids={}", app_id)).unwrap().text().unwrap();
    ...
    pub fn from_query(search_query: &str) -> Vec<SteamGame> {
        let response = blocking::get("https://api.steampowered.com/ISteamApps/GetAppList/v2/").unwrap().text().unwrap();
        ...
        for app in app_ids {
            ...
            let app = match SteamGame::from_appid(app) {
...
pub fn get_steamapps() -> Vec<SteamApp> {
    let response = blocking::get("https://api.steampowered.com/ISteamApps/GetAppList/v2/").unwrap();
